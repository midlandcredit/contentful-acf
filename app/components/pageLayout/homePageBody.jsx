    ...
    <div className='text-center text-clamp-home-body'>
      ...
      <div className='flex flex-row flex-wrap justify-center w-[100%] m-[auto] py-[30px] gap-[10px]'>
        {data.attachmentsCollection.items.map((item, index) => {
          return (
            <div key={index} className='flex flex-col flex-wrap tablet:justify-center justify-between items-center basis-[8.5rem] tablet:basis-[12rem] laptop:basis-[18rem] laptop:h-[290px] rounded-[10px] bg-[#DFE8F8] p-[13px] py-[15px] px-[20px] mx-[5px] my-[5px] text-midnight-blue font-semibold'>
              <div className='w-[30%] m-[5px] laptop:mb-[30px]'>
              <img src={item.image.url} />
              </div>
              <p className='text-clamp-title-cta'>{item.title}</p>
              {item.phoneNumber && <div className='text-clamp-title-cta'>{item.phoneNumber.phoneNumber}</div>}
