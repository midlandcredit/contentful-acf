...
export default function FaqCard({data}) {
  ...
  return (
    <div className={`bg-morning-sky-blue my-[20px] px-[10px] rounded-[10px] leading-[normal] flex flex-col justify-center laptop:h-[535px] laptop:px-[50px] tablet:rounded-[50px] ${data.number === 1 ? 'py-[20px]' : 'py-[35px] mb-[10px]'} ${data.number === 1 ? 'text-left' : 'text-center'}`}>
        <div className='px-[20px] mb-[15px]'>
          ...
          <div className={`flex flex-col ${data.number === 1 ? 'flex-col' : 'tablet:flex-col-reverse'}`}>
            <div className={`rounded-[50px] bg-midnight-blue text-[#FFFFFF] text-center px-[15px] py-[13px] w-[65%] tablet:w-[40%] laptop:w-[345px] ${data.number === 1 ? 'm-[0]' : 'm-[auto]'} my-[20px] font-medium text-clamp-faq-button`}>
